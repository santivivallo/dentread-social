...
| Dato correcto, interpretación errada | **Parcial** — `pipeline/referentes.py` rechaza el texto que nombra una magnitud que la fuente no mide (honorario→costo, cobertura→uso, diagnóstico→tratamiento). Cubre las confusiones conocidas de este dominio, no todas las posibles |
